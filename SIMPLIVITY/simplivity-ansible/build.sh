...
exit_code_build_simplivity_ansible=0
...
setup () {
  ...
  if [ -z ${PYTHON_SDK+x} ]; then
    export PYTHON_SDK=../simplivity-python
  fi
...
update_doc_fragments () {
  # NOTE: Set the destination path to copy the simplivity doc fragments in an env var named DOC_FRAGMENTS_PATH.
  # Otherwise, the destination will be defined automatically.
  local docfragments="build-doc/module_docs_fragments/simplivity.py"

  if [ "$DOC_FRAGMENTS_PATH" ]; then
    cp -f $docfragments $DOC_FRAGMENTS_PATH
  else
    # Find site packages. If it exists, Simplivity doc fragment will be copied to the discovered path
    ...
    if [ -d "${site_packages}/ansible/utils/module_docs_fragments/" ]; then
      ...
      # Copy Simplivity doc fragments to the Ansible codebase path
...
print_summary () {
  if [ $2 -eq 0 ]; then
    ...
    exit_code_build_simplivity_ansible=$((${exit_code_build_simplivity_ansible}+1))
  fi
}
...
ansible-playbook -i "localhost," --syntax-check examples/*.yaml
...
echo "Done. Your build exited with ${exit_code_build_simplivity_ansible}."
exit ${exit_code_build_simplivity_ansible}
